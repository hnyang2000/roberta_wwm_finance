...
class Trainer:
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # viterbi prediction,在MLM任务时关闭
            # if isinstance(model, torch.nn.DataParallel):
            #     model = model.module
            # path_score, logits = model.crf(logits, masks.byte())
            ...
            if args.eval_accumulation_steps is not None and (step + 1) % args.eval_accumulation_steps == 0:
                ...
                #     all_masks = (
                #         masks if all_masks is None else nested_concat(all_masks, masks, padding_index=-100)
                #     )

                # Set back to None to begin a new accumulation
                losses_host, preds_host, labels_host = None, None, None, None
    ...
    def prediction_step(
        ...
        # attention mask is also needed，在MLM时关闭
        # masks = nested_detach(tuple(inputs.get(name) for name in ['attention_mask']))
        # if len(masks) == 1:
        #     masks = masks[0]
